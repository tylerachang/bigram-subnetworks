...
The tokenized inputs to train the n-gram model should be a txt file where each
line is a space-separated list of token ids.
Minimal usage:

NGRAM_CACHE_DIR = 'ngrams_cache'
NGRAM_PREFIX = 'ngram_identifier'
NGRAM_N = 2  # Bigrams.
VOCAB_SIZE = 50280
ngram_model = NGramModel(NGRAM_CACHE_DIR, NGRAM_PREFIX, NGRAM_N, VOCAB_SIZE,
        reference_path='tokenized_inputs.txt',
        prune_every=999999999, prune_minimum=None)

Then, the function ngram_model.ngram_predict_with_backoff can be used similarly
to a Hugging Face model, taking a tensor of input token ids and outputting
next token bigram probabilities. Notably, when training circuits, the target_fn
can be set to ngram_model.ngram_predict_with_backoff.
